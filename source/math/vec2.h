#ifndef SHADOW_VEC2_INCLUDE
#define SHADOW_VEC2_INCLUDE
...
namespace sh
{
	namespace math
	{
		...
		public:
			...
			vec2(const dimension2d<T>& other) : X(other.Width), Y(other.Height) {}
			...
			vec2<T>& operator=(const dimension2d<T>& other) { X = other.Width; Y = other.Height; return *this; }

			vec2<T> operator+(const vec2<T>& other) const { return vec2<T>(X + other.X, Y + other.Y); }
			vec2<T> operator+(const dimension2d<T>& other) const { return vec2<T>(X + other.Width, Y + other.Height); }
			vec2<T>& operator+=(const vec2<T>& other) { X += other.X; Y += other.Y; return *this; }
			...
			bool equals(const vec2<T>& other, const T tolerance = (T)ROUNDING_ERROR_f32) const
			{
				return core::equals(X, other.X, tolerance) && core::equals(Y, other.Y, tolerance);
			}

			vec2<T>& set(T nx, T ny) { X = nx; Y = ny; return *this; }
			vec2<T>& set(const vec2<T>& p) { X = p.X; Y = p.Y; return *this; }
			...
			T getDistanceFrom(const vec2<T>& other) const
			{
				return vec2<T>(X - other.X, Y - other.Y).getLength();
			}
			...
			{
				return vec2<T>(X - other.X, Y - other.Y).getLengthSQ();
			}
			...
			vec2<T>& rotateBy(f64 degrees, const vec2<T>& center = vec2<T>())
			{
				degrees *= DEGTORAD64;
			...
			inline f64 getAngleWith(const vec2<T>& b) const
			{
			...
			vec2<T> getInterpolated(const vec2<T>& other, f64 d) const
			{
				f64 inv = 1.0f - d;
				return vec2<T>((T)(other.X*inv + X*d), (T)(other.Y*inv + Y*d));
			}
			...
			\return An interpolated vector. This vector is not modified. */
			vec2<T> getInterpolated_quadratic(const vec2<T>& v2, const vec2<T>& v3, f64 d) const
			{
				...
				return vec2<T>((T)(X * mul0 + v2.X * mul1 + v3.X * mul2),
			...
			Note that this is the opposite direction of interpolation to getInterpolated_quadratic()
			*/
			vec2<T>& interpolate(const vec2<T>& a, const vec2<T>& b, f64 d)
			{
