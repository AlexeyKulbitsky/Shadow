...
namespace sh
{
	namespace video
	{
		...
		void GL3RenderPipeline::ParseAttributes()
		{
			...
			SPtr<GL3VertexDeclaration> vertexDeclaration(new GL3VertexDeclaration());
		...
		GL3RenderPipeline::GL3RenderPipeline(const RenderPipelineDescription& description)
		{
			...
			if (!linked)
			{
				...
				SH_ASSERT(0, "Error while linking GL3 shader program! Error message: %s", log);
			}
