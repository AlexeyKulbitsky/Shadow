...
namespace gui
{
	...
	{
		m_button->SetHeight(m_rect.GetHeight());
		...
		m_button->SetPosition(m_rect.upperLeftCorner.x + delta, m_rect.upperLeftCorner.y);
	}
	...
	void SliderWidget::OnButtonPressed()
	{
		sh::Device* device = sh::Device::GetInstance();
		sh::InputManager* inputManager = device->GetInputManager();
		m_startPos = inputManager->GetMousePositionCurrent();
		
		m_dragStarted = true;
		GuiManager::GetInstance()->SetFocusWidget(shared_from_this());
	}

	void SliderWidget::OnButtonReleased()
	{
		m_dragStarted = false;
		if (IsInFocus())
			GuiManager::GetInstance()->SetFocusWidget(nullptr);
