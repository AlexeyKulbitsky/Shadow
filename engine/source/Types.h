...
namespace sh
{
	...
	strings
	*/
#if defined(SH_WCHAR_FILESYSTEM)
	typedef wchar_t fschar_t;
#define SH_TEXT(X) L##X
#else
...
#if defined(_DEBUG)
#if defined(SH_WINDOWS_API) && defined(_MSC_VER) && !defined (_WIN32_WCE)
#if defined(WIN64) || defined(_WIN64) // using portable common solution for x64 configuration
#include <crtdbg.h>
#define SH_DEBUG_BREAK_IF( _CONDITION_ ) if (_CONDITION_) {_CrtDbgBreak();}
#else
#define SH_DEBUG_BREAK_IF( _CONDITION_ ) if (_CONDITION_) {_asm int 3}
#endif
#else
#include "assert.h"
#define SH_DEBUG_BREAK_IF( _CONDITION_ ) assert( !(_CONDITION_) );
#endif
#else
#define SH_DEBUG_BREAK_IF( _CONDITION_ )
#endif
...
#define SH_DEPRECATED_ __declspec(deprecated)
#elif (__GNUC__ > 3 || (__GNUC__ == 3 && __GNUC_MINOR__ >= 1)) // all versions above 3.0 should support this feature
#define SH_DEPRECATED_  __attribute__ ((deprecated))
#else
#define SH_DEPRECATED_
#endif
...
#if defined(SH_WINDOWS_API) && defined(_MSC_VER) && (_MSC_VER >= 1400)
...
#define MAKE_IRR_ID(c0, c1, c2, c3) \
		((sh::u32)(sh::u8)(c0) | ((sh::u32)(sh::u8)(c1) << 8) | \
		((sh::u32)(sh::u8)(c2) << 16) | ((sh::u32)(sh::u8)(c3) << 24 ))
